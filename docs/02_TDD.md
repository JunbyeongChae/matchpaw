...
| AI | Claude API (Anthropic) | API 버전 확정 | 설문 분석·감성 코멘트·체크리스트 생성 3개 기능을 동일 SDK로 통일 |
...
| 외부 API | AI 분석·감성 코멘트·체크리스트 생성 | Claude API / `ANTHROPIC_API_KEY` (서버 전용 환경변수) |
...
요청 수신
      │
next/headers cookies()로 matchCount + matchDate 읽기
      │
오늘 날짜와 matchDate 비교
  ├── 다른 날 → matchCount 초기화 (오늘부터 1회로 시작)
  └── 같은 날 → matchCount 확인
            ├── 2 미만 → 매칭 진행, matchCount + 1
            └── 2 이상 → 429 응답 (횟수 초과, 로그인 유도)
```
...
export async function callClaude(prompt: string, maxTokens = 1024): Promise<string> {
  const response = await anthropic.messages.create({
    model: 'claude-haiku-4-5-20251001', // OQ-01 참조: Haiku vs Sonnet 미결
...
// 배치 코멘트 생성 시 호출 예시
// max_tokens: 2048 — 동물 20마리 × 코멘트 ~80 토큰 = 최대 1,600 토큰
const comments = await callClaude(batchPrompt, 2048);
```
...
| Claude API 매칭 결과 | 캐싱 없음 (설문 응답마다 고유) | 설문 조합이 다양해 캐싱 효율이 낮음 |
| Claude API 체크리스트 | DB 저장 (회원) / 캐싱 없음 (비회원) | 회원은 DB에서 재조회, 비회원은 세션 내 재생성 |
...
외부 API 실패
  ...
  └── Claude API 실패      → 타임아웃 5초, 503 에러 응답, 재시도 버튼 표시
```
...
| 찜 데이터 저장 | 스냅샷 저장 | 원본 API desertionNo만 FK로 저장 | 공공 API 원본이 삭제되어도 찜 목록 데이터를 보존하기 위해 |
| 감성 코멘트 생성 | 배치 단일 호출 | 동물 1마리당 1회 호출 | 20마리 × 1회 = 21 API 호출 → RPM 제한 위험. 배치 호출로 총 2회로 축소 |
...
| Claude API 응답 지연 (> 5초) | 中 | 中 | Route Handler에서 5초 타임아웃 설정. 초과 시 503 응답 + 클라이언트 재시도 버튼 |
...
| # | 항목 | 현재 상태 | 결정 필요 시점 |
|---|---|---|---|
| OQ-01 | Claude 모델 선택 (Haiku vs Sonnet) — 응답 품질과 API 비용 트레이드오프 | 미결 | 1주차 API 테스트 전 |
| OQ-02 | 공공 유기동물 API 일일 호출 한도 수치 확인 | 미결 | 1주차 API 키 발급 시 |
| OQ-03 | 비교 테이블 표시 항목 확정 (크기·나이·특성·보호소 외 추가 여부) | 미결 | 3주차 찜 기능 구현 전 |
