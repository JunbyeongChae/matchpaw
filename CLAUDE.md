...
유기동물 입양 AI 매칭 플랫폼. 라이프스타일 설문 5문항 → Gemini API 분석 → 맞춤 유기동물 추천 + 입양 체크리스트 자동 생성.
...
1. **외부 API는 Route Handler에서만 호출한다.** Gemini API, 공공 유기동물 API를 Client Component에서 직접 호출하지 않는다.
...
GEMINI_API_KEY=        # Google Gemini API (aistudio.google.com/app/apikey)
```
...
| 6/13–14 | Phase 2 | `lib/prisma.ts`, `lib/animalApi.ts`, `lib/geminiApi.ts` 배치 호출 프로토타입 |
...
- **타입 안정성**: 공공 API, Gemini API 응답은 반드시 `types/`에 명시적 타입 정의.
...
이 프로젝트에서 Gemini API를 호출할 때는 아래 모델을 사용한다.
...
- [ ] `GEMINI_API_KEY`, `ANIMAL_API_KEY`가 `NEXT_PUBLIC_` 없이 서버 전용으로만 사용됨
